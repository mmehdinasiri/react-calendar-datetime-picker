...
describe('useCalendarState', () => {
  ...
  it('initializes with default values', () => {
    ...
    expect(result.current.state.displayMonth).toEqual({ year: 2023, month: 5, day: 15 })
  })
  ...
  it('initializes with initValue', () => {
    ...
    expect(result.current.state.displayMonth).toEqual({ year: 2023, month: 1, day: 1 })
  })
  ...
  it('selects range (start then end)', () => {
    ...
    expect(result.current.state.selectedValue).toEqual({ from: start, to: null })
  ...
  it('navigates months', () => {
    ...
    expect(result.current.state.displayMonth).toEqual(expect.objectContaining({ year: 2023, month: 6 }))
    
    act(() => {
      result.current.actions.navigateMonth('prev')
    })
    expect(result.current.state.displayMonth).toEqual(expect.objectContaining({ year: 2023, month: 5 }))
  })
  ...
  it('updates time for single date', () => {
    ...
    const { result } = renderHook(() => 
      useCalendarState({ locale: 'en', type: 'single', onChange, initValue, withTime: true })
    )
  ...
  it('adds system time if withTime is true', () => {
    const { result } = renderHook(() => 
      useCalendarState({ locale: 'en', type: 'single', onChange, withTime: true })
    )
    ...
    expect(result.current.state.selectedValue).toEqual({ ...day, hour: 12, minute: 0 })
  })

  it('syncs with external initValue changes', () => {
    const { result, rerender } = renderHook(
      (props) => useCalendarState(props),
      { initialProps: { locale: 'en', type: 'single', onChange, initValue: null as Day | null } }
    )
    ...
    expect(result.current.state.displayMonth).toEqual(expect.objectContaining({ year: 2023, month: 10 }))
  })
})
