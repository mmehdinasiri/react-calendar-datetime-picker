...
describe('useKeyboardNavigation', () => {
  ...
  beforeEach(() => {
    ...
    containerRef = {
      current: document.getElementById('container') as HTMLDivElement
    }
  ...
  it('handles ArrowRight (next day)', () => {
    ...
    expect(onFocusedDateChange).toHaveBeenCalledWith(
      expect.objectContaining({ year: 2023, month: 1, day: 16 })
    )
  })
  ...
  it('handles ArrowLeft (prev day)', () => {
    ...
    expect(onFocusedDateChange).toHaveBeenCalledWith(
      expect.objectContaining({ year: 2023, month: 1, day: 14 })
    )
  })
  ...
  it('handles ArrowDown (next week)', () => {
    ...
    expect(onFocusedDateChange).toHaveBeenCalledWith(
      expect.objectContaining({ year: 2023, month: 1, day: 22 })
    )
  })
  ...
  it('handles ArrowUp (prev week)', () => {
    ...
    expect(onFocusedDateChange).toHaveBeenCalledWith(
      expect.objectContaining({ year: 2023, month: 1, day: 8 })
    )
  })
  ...
  it('handles End (end of month)', () => {
    ...
    expect(onFocusedDateChange).toHaveBeenCalledWith(
      expect.objectContaining({ year: 2023, month: 1, day: 31 })
    )
  })
  ...
  it('handles PageUp (prev month)', () => {
    ...
    expect(onFocusedDateChange).toHaveBeenCalledWith(
      expect.objectContaining({ year: 2022, month: 12, day: 15 })
    )
  })
  ...
  it('handles PageDown (next month)', () => {
    ...
    vi.advanceTimersByTime(0)
    expect(onFocusedDateChange).toHaveBeenCalledWith(
      expect.objectContaining({ year: 2023, month: 2, day: 15 })
    )
  })
