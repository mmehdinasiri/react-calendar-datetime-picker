...
function parsePerformanceResults(testResultsPath) {
  ...
  try {
    console.log('✅ Reading metrics from performance-metrics.json')
    ...
    console.log(`📊 Loaded ${metricCount} performance metrics`)

    // Log any unusual values
    Object.entries(metrics).forEach(([key, value]) => {
      if (typeof value !== 'number' || isNaN(value) || value < 0) {
        console.warn(`⚠️ Unusual metric value for ${key}: ${value}`)
      }
    })

    return metrics
  } catch (error) {
    console.error(`❌ Failed to parse performance metrics: ${error.message}`)
    throw error
  }
}
...
switch (command) {
  ...
  default:
    console.log('Usage:')
    console.log(
      '  node generate-performance-report.js report [results.json] [output.md]'
    )
    console.log(
      '  node generate-performance-report.js compare [baseline.json] [current.json] [output.md]'
    )
    console.log('')
    console.log('Examples:')
    console.log('  node generate-performance-report.js report')
    console.log(
      '  node generate-performance-report.js compare baseline.json performance-metrics.json'
    )
    break
}
